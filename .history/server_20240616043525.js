...
io.on('connection', async(socket) => {
  ...
  socket.on('sendMessage', async (data) => {
    ...
    const messageData =await createChatModel.create({ sender, receiver, message });

    messageData.save().then(() => { 
      io.to(receiver).emit('newMessage', messageData);
      io.emit('newMessage', messageData); // إرسال الرسالة للمرسل أيضا
    });
  });
   createChatModel.find().then((messages) => {
