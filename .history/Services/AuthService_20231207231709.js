...
exports.restCodeSent = expressAsyncHandler(async(req , res , next)=>{
  var ciphertext = CryptoJS.AES.encrypt(req.body.restCode, "secret key 123").toString();
  const user = await createUsersModel.findOne({ passwordRResthashedCode: ciphertext });
  if (!user || user.passwordRRestExpires !== Date.now * 60 ) {
    return next(new ApiError(`Incorrect Rest Code`));
  }
})
