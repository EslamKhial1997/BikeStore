...
exports.forgetPassword = expressAsyncHandler(async (req, res, next) => {
  ...
  const digitCode = Math.floor(1000 + Math.random() * 9000000).toString();
  ...
  await user.save();
  const massage = `Hi ${user.email},\n We Receved Password To Rest The Password on Bike Store \n <div>${digitCode}</div> Enter This Digits Code`;
  await sendCode({
