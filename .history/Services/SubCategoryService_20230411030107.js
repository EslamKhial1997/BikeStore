...
exports.getSubCategories = asyncHandler(async (req, res) => {
  ...
  if(req.params.categoryId) filterSubCategory={categoryId:req.params.categoryId}
  const page = req.query.page * 1 || 1;
  const limit = req.query.limit * 1 || 10;
  const skip = (page - 1) * limit;
  const getCategories = await createSubCategoryModel.find(filterSubCategory).skip(skip).limit(limit)
  res
    .status(200)
