...
exports.createCart = expressAsyncHandler(async (req, res, next) => {
  ...
  if (!cart) {
    ...
    const totalCartPrice = cart.cartItems[productExists];
    totalCartPrice.price = totalCartPrice.product.price * totalCartPrice.quantity / totalCartPrice.quantity;
    cart.cartItems = totalCartPrice;
    console.log(totalCartPrice.product.price);
  }
  await cart.save();
  res.status(200).json({
    status: "success",
