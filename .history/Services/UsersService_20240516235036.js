...
exports.updateRole = expressAsyncHandler(async (req, res, next) => {
  const user = await createUsersModel.findByIdAndUpdate(req.params.id, {
    role: req.body.role,
  } , {
    new: true,
  });
  if (!user) {
    return next(new ApiError(`User ${req.params.id}`));
  }
  if (!user || !(await bcrypt.compare(req.body.passwordDB, user.passwordDB))) {
    return next(new ApiError("InCorrect password Or Email"));
  }
