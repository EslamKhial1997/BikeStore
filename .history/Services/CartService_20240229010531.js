...
exports.getCart = expressAsyncHandler(async (req, res, next) => {
  ...
  const cart = new FeatureApi(createCartModel.find({ user: req.user._id }), req.query).Paginate()
  if (!cart) {
    return next(
...
console.log(cart);
  ...
  res
    .status(201)
    .json({ results: getDoc.length, PaginateResult:getDoc.cartItems.length, data: getDoc });
});
  // res.status(200).json({
