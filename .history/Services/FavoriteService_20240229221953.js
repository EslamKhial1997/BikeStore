...
exports.createFavorite = expressAsyncHandler(async (req, res, next) => {
  ...
  if (!favorite) {
    ...
    await favorite.cartItems.push({ product: req.body.product });
  }
  await favorite.save();
  res.status(200).json({
    status: "success",
...
exports.getCart = expressAsyncHandler(async (req, res, next) => {
  const favorite = await createCartModel.findOne({ user: req.user._id });
...
exports.deleteSpecificCartItem = expressAsyncHandler(async (req, res, next) => {
  const favorite = await createCartModel.findOneAndUpdate(
    { user: req.user._id },
    {
