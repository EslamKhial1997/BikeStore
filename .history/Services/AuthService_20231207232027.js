...
exports.restCodeSent = expressAsyncHandler(async(req , res , next)=>{
  ...
  const user = await createUsersModel.findOne({ passwordRResthashedCode: ciphertext  , passwordRRestExpires:{$gt: Date.now}});
  if (!user) {
    return next(new ApiError("Rest Code is Invalid Or Exp"));
  }
})
