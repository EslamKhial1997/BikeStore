...
exports.updatePaidStatus = expressAsyncHandler(async (req, res, next) => {
  const order = await createOrderModel.findByIdAndUpdate(
    req.params.id,
    {
  ...
  if (!order) {
    return next(new ApiError(`Order ${req.params.id} Not Found`));
  }

  res.status(200).json({ data: order });
});
exports.updateDeliverdStatus = expressAsyncHandler(async (req, res, next) => {
  const order = await createOrderModel.findByIdAndUpdate(
    req.params.id,
    {
