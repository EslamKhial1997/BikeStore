...
exports.getCart = expressAsyncHandler(async (req, res, next) => {
  ...
  const countDocs = await createCartModel.cartItems.countDocuments();
    const ApiFeatures = new FeatureApi(createCartModel.findOne({ user: req.user._id }), req.query)
      .Fillter()
      .Sort()
      .Fields()
      .Search()
      .Paginate(createCartModel.cartItems);
