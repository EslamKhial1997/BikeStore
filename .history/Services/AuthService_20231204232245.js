...
exports.forgetPassword = expressAsyncHandler(async (req, res, next) => {
  ...
  await user.save();
  const massage = `Hi ${user.email} We Receved Password To Rest The Password on Bike Store ${digitCode} Enter This Digits Code`;
  await sendCode({
    digist: ciphertext,
  ...
  res
    .status(200)
    .json(
      { status: "success", massage: "Rest Code Sent successfully" },
