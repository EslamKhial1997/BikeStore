...
exports.passwordDashboard = expressAsyncHandler(async (req, res, next) => {
  ...
  user.passwordDB = req.body.passwordDB;
...
exports.updatePasswordDashboard = expressAsyncHandler(async (req, res, next) => {
  const updatepasswordDashboardById = await createUsersModel.findByIdAndUpdate(
    req.params.id,
    {
      passwordDB:await bcrypt.hash(req.body.password, 12),
...
exports.updateLoggedUserPasswordDashboard = expressAsyncHandler(async (req, res) => {
  const user = await createUsersModel.findByIdAndUpdate(
    req.user._id,
    {
